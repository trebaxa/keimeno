...
<div class="form-group">
    <label>Blog Auswahl:</label>
    <select class="form-control custom-select" name="PLUGFORM[themeid]">
        <% foreach from=$WEBSITE.PLUGIN.result.blogs item=row %>
            <option <% if ($WEBSITE.node.tm_plugform.themeid==$row.ID) %>selected<%/if%> value="<%$row.ID%>"><%$row.LABEL%></option>
        <%/foreach%>
    </select>
</div>
...
<div class="form-group">
    ...
    <input maxlength="4" type="text" class="form-control" name="PLUGFORM[foto_width]" value="<% $WEBSITE.node.tm_plugform.foto_width|hsc %>">
</div>

<div class="form-group">
    <label>Foto Thumb H&ouml;he:</label>
    <input maxlength="4" type="text" class="form-control" name="PLUGFORM[foto_height]" value="<% $WEBSITE.node.tm_plugform.foto_height|hsc %>">
</div>
