...
<ul>
<li id="gallerytreeroot" data-haschildren="1" ><a id="ident-0" href="javascript:void(0)"  data-tid="0">Alben</a>
    <ul>
<% function name="gal_tree" %>
   ...
   <li id="gallerynode-<%$element.id%>" data-haschildren="<% if (!empty($element.children)) %>1<%else%>0<%/if%>" data-tid="<%$element.id%>" data-isadmin="<%$element.admin%>" <% if (empty($element.children)) %>data-jstree='{"icon":"glyphicon glyphicon-file"}'<%/if%>>   
    ...
    <a href="#" id="ident-<%$element.id%>" title="<%$element.catlabel|hsc%>"><%$element.groupname%>(<%$element.piccount%>)
   </a>
        ...
        var items = {
            ...
            editItem: { // The "edit" menu item
                ...
                action: function () {
                    ...
                    if(!sel.length) { return false; }      
                    window.location.href='run.php?epage=gallery.inc&toplevel=<%$GET.toplevel%>&cmd=edit&id='+$('#'+sel[0]).data('tid');  
                }
            }
        };
         ...
         core : {
          ...
          "contextmenu": {items: customMenuGallery}
           ...
           if ($('#'+link_id).data('haschildren')==0 && $('#'+link_id).data('tid')>0) {                     
                simple_load('js-fotolist','<%$eurl%>&cmd=load_pics&gid='+$('#'+link_id).data('tid'));
                return;
           } else {
            expand_node_webtree( link_id,'gallerytreecont');
            if ($('#'+link_id).data('tid')>0) simple_load('js-fotolist','<%$eurl%>&cmd=load_pics&gid='+$('#'+link_id).data('tid'));
           }
            ...
            $.ajax({
			     url: '?epage=gallery.inc&cmd=add_website_tree',
                 data: { 'position' : data.position, 'FORM[description]' : data.node.text, 'FORM[parent]' : $('#'+data.node.parent).data('tid') },
                 async :false,
                 dataType :'json'
                 })
				.done(function (d) {
                    window.location.href='run.php?epage=gallery.inc&toplevel=<%$GET.toplevel%>&cmd=edit&id='+d.id;
				})
