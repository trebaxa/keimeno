<h3>{LBL_OTTHEME}</h3>
<div class="btn-group">
    <a href="#" data-toggle="modal" data-target="#newprogotimer" class="btn btn-secondary">Neues Thema</a>
</div>
<% if (count($OTIMER.otgroups)>0) %>
...
<table class="table table-striped table-hover" id="ot-group-table" >
        ...
        <tbody>
		 <% foreach from=$OTIMER.otgroups item=row %>		  
		  <tr>
...
<div class="modal fade" id="newprogotimer" tabindex="-1" role="dialog" aria-labelledby="newprogotimerLabel" aria-hidden="true">
  <div class="modal-dialog">
    ...
    <form action="<%$PHPSELF%>" method="post" class="form">
            <input type="hidden" name="epage" value="<%$epage%>">
            <input type="hidden" name="cmd" value="add_otgroup">
      <div class="modal-header">
        <h5 class="modal-title" id="newprogotimerLabel">Neues Programm</h5>
      ...
      <div class="modal-body">
        <div class="form-group">
            ...
            <input type="text" required="" placeholder="Thema Name" class="form-control" name="FORM[groupname]">
        </div>    
      </div>
