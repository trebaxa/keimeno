...
<div class="form-group">
    ...
    <select class="form-control custom-select" id="js-flx-groupid">
...
<div id="tab-flxtpl-2" class="tabvisi" <% if (count($FLEXTEMP.flextpl.flexvars)==0 && count($FLEXTEMP.flextpl.datasetvarsdb)>0)%>style="display:block"<%/if%>>
   <div class="mt-lg">
    <div class="btn-group">
        <button class="btn btn-secondary" type="button" onclick="reload_dataset(1,<%$FLEXTEMP.flextpl.group.id%>)"><i class="fa fa-refresh"></i>&nbsp;alle anzeigen</button>
        <button class="btn btn-secondary" type="button" onclick="simple_load('js-after-plugin-editor','<%$eurl%>cmd=show_addds&gid=<%$FLEXTEMP.flextpl.group.id%>&content_matrix_id=<% $GET.content_matrix_id %>&flxid=<%$GET.flxid%>')"><i class="fa fa-plus"></i>&nbsp;Neuer Datensatz</button>    
    </div>
                ...
                <% foreach from=$FLEXTEMP.flextpl.dataset item=row %>
                    <tr>                     
                            ...
                            <% if ($dskey!='id' && $dskey!='ds_order')%>
                             <td <% if ($row.column.$dskey.v_type=='img') %>class="col-md-1"<%/if%>>                           
                                <% if ($row.column.$dskey.v_type=='img') %>
                                    <img class="img-fluid" src="<%$row.column.$dskey.thumb%>?a=<%$randid%>" />
                       ...
                       <td class="col-md-2 text-right">
                         <div class="btn-group">
                            <button type="button" class="btn btn-secondary" onclick="simple_load('js-after-plugin-editor','<%$eurl%>rowid=<%$row.row.id%>&cmd=show_edit_dataset&content_matrix_id=<% $GET.content_matrix_id %>&flxid=<%$GET.flxid%>&gid=<%$FLEXTEMP.flextpl.group.id%>')"><i class="far fa-edit"></i></button>
