      <td><% assign var=vc value=$smarty.foreach.vloop.iteration+$REQUEST.start %><%$vc%></td>
        <td><input type="checkbox" id="idvideo" name="VIDEOIDS[]" value="<% $ytvideo.v_videoid %>"><input type="hidden" name="VIDS[]" value="<% $ytvideo.v_videoid %>"></td>
        ...
        <td class="text-center"><input <% if ($ytvideo.v_frontpage==1) %>checked<%/if%> type="checkbox" value="1" name="VID[<%$ytvideo.v_videoid%>][v_frontpage]"></td>
        <td class="text-center"><% if ($ytvideo.v_frontpage==1) %>
        <input type="text" class="form-control" size="3" maxlength="3" value="<%$ytvideo.v_order%>" name="VID[<%$ytvideo.v_videoid%>][v_order]">
        <%else%>-<%/if%></td>          
