...
class phpVimeoAPI {
    ...
    private function _generateSignature($params, $request_method = 'GET', $url = self::API_REST_URL) {
        ...
        // Make the key
        $key_parts = array($this->_consumer_secret, ($this->_token_secret) ? $this->_token_secret : '');
    ...
    private function _request($method, $call_params = array(), $request_method = 'GET', $url = self::API_REST_URL, $cache = true, $use_auth_header = true) {
        ...
        if (strtoupper($request_method) == 'GET') {
            ...
            $curl_opts = array(CURLOPT_RETURNTRANSFER => true, CURLOPT_TIMEOUT => 30);
        }
        else
        ...
        if (!empty($method)) {
            ...
            if ($response->stat == 'ok') {
                ...
                if ($response->err) {
                    throw new VimeoAPIExceptClass($response->err->msg, $response->err->code);
                }
    ...
    public function getRequestToken($callback_url = 'oob') {
        $request_token = $this->_request(null, array('oauth_callback' => $callback_url), 'GET', self::API_REQUEST_TOKEN_URL, false, false);
    ...
    public function upload($file_path, $use_multiple_chunks = false, $chunk_temp_dir = '.', $size = 2097152, $replace_id = null) {
        ...
        if ($quota->user->upload_space->free < $file_size) {
            throw new VimeoAPIExceptClass('The file is larger than the user\'s remaining quota.', 707);
        }
        ...
        if ($file_size > $rsp->ticket->max_file_size) {
            throw new VimeoAPIExceptClass('File exceeds maximum allowed size.', 710);
        }
        ...
        if ($use_multiple_chunks) {
            ...
            for ($i = 0; $i < $number_of_chunks; $i++) {
                ...
                $chunks[] = array('file' => realpath($chunk_file_name), 'size' => filesize($chunk_file_name));
            }
        }
        else {
            $chunks[] = array('file' => realpath($file_path), 'size' => filesize($file_path));
        }
        ...
        foreach ($chunks as $i => $chunk) {
            ...
            // Generate the OAuth signature
            $params = array_merge($params, array('oauth_signature' => $this->_generateSignature($params, 'POST', self::API_REST_URL), 'file_data' => '@' . $chunk['file']
        ...
        // Complete the upload
        $complete = $this->call('vimeo.videos.upload.complete', array('filename' => $file_name, 'ticket_id' => $ticket));
        ...
        if ($complete->stat == 'ok') {
            ...
            if ($complete->err) {
                throw new VimeoAPIExceptClass($complete->err->msg, $complete->err->code);
            }
    }
    ...
    public static function url_encode_rfc3986($input) {
        if (is_array($input)) {
            return array_map(array('phpVimeoAPI', 'url_encode_rfc3986'), $input);
        }
        else
            if (is_scalar($input)) {
...
class VimeoAPIExceptClass extends Exception {
}
