    ...
    function parse_mmenus($params) {
        ...
        if (strstr($html, '{TMPL_MMENU_')) {
            ...
            foreach ($tpl_tag[0] as $key => $wert) {
                ...
                $MMENU = $this->load_menu((int)$PLUGIN_OPT['menuid']);

                # Load nested Menu
                $arr = $this->load_mmenu_matrix_fe((int)$PLUGIN_OPT['menuid'], $this->GBL_LANGID);
                $this->menu_opt_fe($arr);
                $nested_menu = $this->nested->create_result_and_array_by_array($arr, 0, 0, -1);
    ...
    function parse_mmenu_manuel($params) {
        ...
        if (strstr($html, '{TMPL_MENU_')) {
            ...
            foreach ($tpl_tag[0] as $key => $wert) {
                ...
                # Load nested Menu
                $arr = $this->load_mmenu_matrix_fe($menu_id, $this->GBL_LANGID);
                $this->menu_opt_fe($arr);
                $nested_menu = $this->nested->create_result_and_array_by_array($arr, 0, 0, -1);
