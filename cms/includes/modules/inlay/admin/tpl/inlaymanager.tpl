<% if ($aktion=='' || $cmd=='ax_show_all') %>
    ...
    <%include file="cb.panel.header.tpl" title="Inlays"%>
       <div class="btn-group">
        <a class="btn btn-secondary ajax-link" href="<%$eurl%>cmd=ax_show_all"><i class="fa fa-table"></i> Alle anzeigen</a>
        <a class="btn btn-secondary" data-toggle="modal" data-target="#meinModal"><i class="fa fa-plus"></i> Neues Inlay anlegen</a>
       </div>
...
<%include file="cb.panel.header.tpl" title="System Templates auf andere Sprachen replizieren"%>
        ...
        <form class="stdform form-inline  mb-2" method="POST" action="<%$PHPSELF%>">
            ...
            <div class="form-group">
                ...
                <select id="relaung"class="form-control custom-select" name="langid">
...
<% if ($aktion=='edit' || $cmd=='ax_edit') %>
 ...
 <%include file="cb.panel.header.tpl" title=$TPLOBJ.description%>
        ...
        <form class="stdform" method="post" action="<%$PHPSELF%>" enctype="multipart/form-data">
            ...
            <div class="form-feet">
                <a class="btn btn-secondary" href="<%$PHPSELF%>?epage=<%$epage%>&aktion=replicateland&id=<% $TPLOBJ.formcontent.tid %>&uselang=<% $GET.uselang %>">Inhalt auf alle Sprachen replizieren</a>
                <%$subbtn%>
            </div><!-- /.form-feet -->
        </form>
