    ...
    <table class="table table-striped">
        <thead>
            <tr>
                ...
                <th><a href="kreg.php?start=<%$GET.start%>&col=email_notpublic&dc=<% $paging.flipped_dc %>">Email Office</a></th>
                <th>{LA_TELEFON}</th>
        ...
        <% foreach from=$CUST.table item=row %>
            <tr>
                <td><a onClick="showPageLoadInfo();" href="kreg.php?cmd=edit&kid=<% $row.kid %>"><% $row.kid %></a></td>
                <td><a href="kreg.php?cmd=edit&kid=<% $row.kid %>"><% $row.nachname %></a></td>
                <td><a href="kreg.php?cmd=edit&kid=<% $row.kid %>"><% $row.vorname %></a></td>
                <td><% mailto address=$row.email encode="hex" %></td>
