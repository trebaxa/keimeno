...
if (isset($GLOBALS["emailmsgclass_php"])) {
    return;
} //** onlyinclude once.
...
define("EmailXMailer", "PHP-EMAIL " . $gbl_config['adr_firma']);
...
class Email {
    ...
    function spamSafe($text) {
        $text = preg_replace("/(\n+|\r+|%0A|%0D)/i", '', $text);
        return preg_replace("/(content-type:|bcc:|cc:|to:|from:)/im", "", $text);
    }

    //** Create a new email message with the parameters provided.

    function __construct($to = null, $from = null, $subject = null, $headers = null, $from_name = null) {
        $this->To = $this->spamSafe($to);
        $this->From = $this->spamSafe($from);
    ...
    function Send() {
        ...
        $headers = "Date: " . date("r", time()) . EmailNewLine . "From: " . ((strlen(trim(strval($this->From_name))) > 0) ? $this->From_name . ' <' . $this->From . '>' :
            $this->From) . EmailNewLine;
        ...
        $headers .= "X-Mailer: " . EmailXMailer . EmailNewLine . "MIME-Version: 1.0" . EmailNewLine . "Content-Type: $baseContentType; " . "boundary=\"$theboundary\"" .
        ...
        if ($isMultipartAlternative) {
            ...
            $thebody = "--$theboundary" . EmailNewLine . $this->Attachments["text"]->ToHeader() . EmailNewLine . "--$theboundary" . EmailNewLine . $this->Attachments["html"]->
                ToHeader() . EmailNewLine;
        }
        //** if either only html or text email add the content to the email body.
            ...
            $thebody = "--$theboundary" . EmailNewLine . "Content-Type: $theemailtype; charset=$this->Charset" . EmailNewLine . "Content-Transfer-Encoding: 8bit" .
                EmailNewLine . EmailNewLine . $this->Content . EmailNewLine . EmailNewLine;
            ...
            foreach ($this->Attachments as $attachment) {
                ...
                if ($attachment != null) {
                    $thebody .= "--$theboundary" . EmailNewLine . $attachment->ToHeader() . EmailNewLine;
                }
            }
        }
        ...
        $headers = str_replace(EmailNewLine . EmailNewLine, EmailNewLine, $headers);
        return mail($this->To, $this->Subject, $thebody, $headers, "-f" . $this->From);
    }
}
...
class EmailAttachment {
    ...
    function __construct($pathtofile = null, $mimetype = null) {
        ...
        if ($mimetype == null || strlen(trim($mimetype)) == 0) {
            $ext = strtolower(strrchr($pathtofile, '.'));
            if ($ext == "pdf")
    ...
    function ToHeader() {
        ...
        if (!$this->HasLiteralContent()) {
            $header .= " name=\"" . basename($this->FilePath) . "\"" . EmailNewLine . "Content-Disposition: attachment; filename=\"" . basename($this->FilePath) . "\"";
        }
        $header .= EmailNewLine;
        ...
        $header .= chunk_split(base64_encode($attachmentData), 76, EmailNewLine) . EmailNewLine;
